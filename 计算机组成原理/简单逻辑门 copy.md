...
　　手头拥有的最基本材料为：**电和晶体管**（也可以使用电子管，但技术旧且效率低）。因计算机采用二进制，而数学中的布尔代数为二进制信息表示和计算提供有力的数学支撑。
...
　　注意，将全加器直接串联起来构成的串行进位加法器（也称行波进位加法器），**尤其懒惰和低效**。现代电子所用的加法器，经过优化，速度更快。**但无论如何，我们已经拥有加法器了。**
